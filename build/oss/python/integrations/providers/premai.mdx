...
To use Prem Templates natively with LangChain, you need to pass an id the `HumanMessage`. This id should be the name the variable of your prompt template. the `content` in [`HumanMessage`](https://reference.langchain.com/python/langchain/messages/HumanMessage) should be the value of that variable.
