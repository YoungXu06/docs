...
Pass one or more of the following stream modes as a list to the [`stream`](https://reference.langchain.com/javascript/langchain-langgraph/CompiledStateGraph) method:
...
To stream agent progress, use the [`stream`](https://reference.langchain.com/javascript/langchain-langgraph/CompiledStateGraph) method with `streamMode: "updates"`. This emits an event after every agent step.
