...
Notice that the `AIMessage` has a [`tool_calls`](https://reference.langchain.com/python/langchain/messages/AIMessage) attribute. This contains in a standardized `ToolCall` format that is model-provider agnostic.
