...
This guide explains how to add semantic search to your deployment's cross-thread [store](/oss/langgraph/persistence#memory-store), so that your agent can search for memories and other documents by semantic similarity.
...
* `langchain >= 0.3.8` (if you use the `"provider:model"` shorthand below).
...
## Querying from an external client

You can also query the store from outside your graph using the LangGraph SDK:
