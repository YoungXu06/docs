...
You only need to **provide the checkpointer when compiling the parent graph**. LangGraph will automatically propagate the checkpointer to the child subgraphs. Subgraph checkpoints are stored under a namespace derived from the node name (e.g., `node_1:`). This is the most common pattern for enabling persistence.

With automatic propagation, you can access subgraph state externally via the `subgraphs` option on the get state method. See [View subgraph state](#view-subgraph-state) for details.
...
If you want the subgraph to **have its own internal memory** that persists across parent graph invocations, compile it with `checkpointer=True` (Python) or `checkpointer: true` (JS). This gives the subgraph an independent checkpoint history within the parent's checkpointer storage. This is useful in [multi-agent](/oss/javascript/langchain/multi-agent) systems where you want agents to keep track of their own internal message histories across invocations:
...
When you enable [persistence](/oss/javascript/langgraph/persistence), you can [inspect the graph state](/oss/javascript/langgraph/persistence#checkpoints) (checkpoint) via the appropriate method. To include subgraph state in the result, pass the subgraphs option. The returned `StateSnapshot` includes nested `StateSnapshot` objects accessible via `.tasks[<task_index>].state`.



```typescript
const state = await graph.getState(config, { subgraphs: true });
const subgraphState = state.tasks[0].state; // nested StateSnapshot
```


<Note>
Subgraph state is available whenever the parent graph has a checkpointer and the subgraph node has pending or completed tasks — not only during interrupts.
</Note>

### Access subgraph state at an interrupt

A common use case is inspecting subgraph state when the subgraph is [interrupted](/oss/javascript/langgraph/interrupts). While the graph is paused, you can view the subgraph state, and then resume execution.
...
<Accordion title="View interrupted subgraph state">
  ...
  // Graph will pause at the interrupt
  await graph.invoke({ foo: "" }, config);

  // View subgraph state while interrupted
  const state = await graph.getState(config, { subgraphs: true });
...
You can retrieve the full checkpoint history of a subgraph by passing the subgraph's `StateSnapshot` config to the state history method. This is useful for debugging or auditing subgraph execution.
...
const childHistory = [];
for await (const snapshot of graph.getStateHistory(subgraphConfig)) {
  childHistory.push(snapshot);
}
```
...
You can update subgraph state by passing the subgraph's state config to the update state method. This is useful for modifying subgraph state during [human-in-the-loop](/oss/javascript/langgraph/interrupts) workflows.
...
await graph.updateState(state.tasks[0].state.config, { foo: "newValue" });
```

You can also specify `asNode` to control which node the update is applied as:
...
### Multi-level nested state access

<Accordion title="Access state across multiple levels of subgraphs">
  For graphs with multiple levels of nesting (parent → child → grandchild), you can traverse the state hierarchy by accessing `.tasks[0].state` at each level.



  ```typescript
  // Get parent state with subgraph state included
  const parentState = await graph.getState(config, { subgraphs: true });

  // Access child subgraph state
  const childState = parentState.tasks[0].state;

  // Access grandchild subgraph state (if the child also has subgraphs)
  const grandchildState = (
    await graph.getState(childState.config, { subgraphs: true })
  ).tasks[0].state;
  ```
