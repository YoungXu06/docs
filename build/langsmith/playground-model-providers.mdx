...
<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
    <a href="#openai-compatible-endpoint" className="flex items-center justify-center gap-1.5 p-2 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 no-underline">
...
OpenAI models excel at general-purpose applications, complex reasoning, and function calling. Authentication requires an [OpenAI API key](https://platform.openai.com/api-keys) or [Azure OpenAI credentials](https://learn.microsoft.com/en-us/azure/ai-services/openai/quickstart).
...
For the current list of available models, see the [OpenAI documentation](https://platform.openai.com/docs/models).
...
Anthropic models excel at long context tasks, safety-critical applications, and detailed analysis. Authentication requires an [Anthropic API key](https://console.anthropic.com/settings/keys).
...
For the current list of available models, see the [Anthropic documentation](https://docs.anthropic.com/claude/docs/models-overview).
...
Google Gemini models excel at multimodal tasks with support for text, images, video, and audio. Authentication requires a [Google AI API key](https://aistudio.google.com/app/apikey).

### Available models

Google offers Gemini models in multiple tiers (Ultra, Pro, Flash) optimized for different use cases.

For the current list of available models, see [Google's Gemini documentation](https://ai.google.dev/models/gemini).
...
Mistral AI models excel at multilingual applications, European data residency, and cost-conscious projects. Authentication requires a [Mistral AI API key](https://console.mistral.ai/api-keys/).

### Available models

Mistral offers models in multiple tiers (Large, Medium, Small) optimized for different performance and cost requirements.

For the current list of available models, see [Mistral's documentation](https://docs.mistral.ai/platform/endpoints/).
...
Fireworks models excel at high-performance inference, production deployments, and cost optimization. Authentication requires a [Fireworks API key](https://fireworks.ai/api-keys).
...
For the current list of available models, see [Fireworks' model documentation](https://docs.fireworks.ai/models).
...
Groq models excel at high-throughput applications, real-time use cases, and cost optimization. Authentication requires a [Groq API key](https://console.groq.com/keys).

### Available models

Groq provides high-speed inference for popular open-source models including Llama, Mixtral, and Gemma variants.

For the current list of available models, see [Groq's model documentation](https://console.groq.com/docs/models).
...
DeepSeek models excel at cost-effective reasoning and coding tasks. Authentication requires a [DeepSeek API key](https://platform.deepseek.com/api_keys).

### Available models

DeepSeek offers general-purpose models, reasoning-optimized models (R-series), and coding-specialized models.

For the current list of available models, see [DeepSeek's documentation](https://platform.deepseek.com/api-docs/).
...
XAI models excel at real-time information and conversational AI. Authentication requires an [xAI API key](https://console.x.ai/).

### Available models

xAI offers Grok models in multiple sizes for different use cases.

For the current list of available models, see [xAI's documentation](https://docs.x.ai/docs).
...
Azure OpenAI provides OpenAI models deployed through Microsoft Azure for enterprise users with compliance requirements and dedicated capacity. Authentication requires [Azure OpenAI credentials](https://learn.microsoft.com/en-us/azure/ai-services/openai/quickstart) (endpoint + API key).
...
For the current list of available models, see [Azure OpenAI documentation](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models).
...
Google Vertex AI provides access to Google's models and other supported models through the Vertex AI platform for enterprise Google Cloud users with global deployment capabilities. Authentication requires [Google Cloud credentials](https://cloud.google.com/vertex-ai/docs/authentication).
...
For the current list of available models, see [Vertex AI documentation](https://cloud.google.com/vertex-ai/docs/generative-ai/models).
...
Amazon Bedrock provides access to foundation models from multiple providers for enterprise AWS users with multi-model flexibility and compliance requirements. Authentication requires [AWS credentials or IAM role](https://docs.aws.amazon.com/bedrock/latest/userguide/security-iam.html).
...
For the current list of available models, see [AWS Bedrock documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/models-supported.html).
...
OpenAI compatible endpoints are ideal for self-hosted models, enterprise deployments, and specialized models. Authentication varies by endpoint (often API key or none).
