...
User-defined fields for access control queries (e.g., `tenant_id`, `owner`) should generally be left **unencrypted** for search and filtering. Additionally, **some system-managed fields will never be encrypted**:
...
As noted above, some fields are excluded from encryption. Unless otherwise noted, these exclusions apply at every level of a nested JSON object, not just the root level.
