...
2. The graph was interrupted before receiving updates from the `tools` node (i.e. a list of [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/ToolMessage))
    ...
    * One of the tools raised an error that wasn't handled by the [`ToolNode`](https://reference.langchain.com/python/langgraph.prebuilt/tool_node/ToolNode) (`"tools"`)
...
* provide [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/ToolMessage) objects that match existing tool calls and call `graph.invoke({'messages': [ToolMessage(...)]})`.
...
or add [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/ToolMessage) objects with `tool_call_ids` that match unanswered tool calls 3. call `graph.update_state(config, {'messages': ...})` with the modified list of messages 4. resume the graph, e.g. call `graph.invoke(None, config)`
