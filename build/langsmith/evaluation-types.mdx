...
Run offline evaluations client-side using the LangSmith SDK ([Python](https://reference.langchain.com/python/langsmith) or [TypeScript](https://reference.langchain.com/javascript/langsmith)) or server-side via the [Prompt Playground](/langsmith/prompt-engineering-concepts#prompt-playground) or by [binding evaluators to a dataset](/langsmith/bind-evaluator-to-dataset).
