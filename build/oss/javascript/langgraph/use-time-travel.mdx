...
1. [Run the graph](#1-run-the-graph) with initial inputs using [`invoke`](https://reference.langchain.com/javascript/langchain-langgraph/CompiledStateGraph) or [`stream`](https://reference.langchain.com/javascript/langchain-langgraph/CompiledStateGraph) methods.
2. [Identify a checkpoint in an existing thread](#2-identify-a-checkpoint): Use the [`getStateHistory`](https://reference.langchain.com/javascript/langchain-langgraph/pregel/Pregel) method to retrieve the execution history for a specific `thread_id` and locate the desired `checkpoint_id`.
...
3. [Update the graph state (optional)](#3-update-the-state-optional): Use the [`updateState`](https://reference.langchain.com/javascript/langchain-langgraph/pregel/Pregel) method to modify the graph's state at the checkpoint and resume execution from alternative state.
...
### 1. Run the graph
To start the workflow, [`invoke`](https://reference.langchain.com/javascript/langchain-langgraph/CompiledStateGraph) is called without any inputs. Note the `thread_id` to track this execution and retrieve its checkpoints later.
...
To continue from a previous point in the graphs run, use [`get_state_history`](https://reference.langchain.com/javascript/langchain-langgraph/pregel/Pregel) to retrieve all the states and select the one where you want to resume execution.
...
For resumings execution from the selected checkpoint, call [`invoke`](https://reference.langchain.com/javascript/langchain-langgraph/CompiledStateGraph) with the config that points to the new checkpoint.
