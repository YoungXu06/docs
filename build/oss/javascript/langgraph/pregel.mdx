...
[`Pregel`](https://reference.langchain.com/javascript/langchain-langgraph/pregel/Pregel) implements LangGraph's runtime, managing the execution of LangGraph applications.

Compiling a [StateGraph](https://reference.langchain.com/javascript/langchain-langgraph/StateGraph) or creating an [entrypoint](https://reference.langchain.com/javascript/langchain-langgraph/entrypoint) produces a [`Pregel`](https://reference.langchain.com/javascript/langchain-langgraph/pregel/Pregel) instance that can be invoked with input.
...
> **Note:** The [`Pregel`](https://reference.langchain.com/javascript/langchain-langgraph/pregel/Pregel) runtime is named after [Google's Pregel algorithm](https://research.google/pubs/pub37252/), which describes an efficient method for large-scale parallel computation using graphs.
...
* [`LastValue`](https://reference.langchain.com/javascript/langchain-langgraph/channels/LastValue): The default channel, stores the last value sent to the channel, useful for input and output values, or for sending data from one step to the next.
...
* [`BinaryOperatorAggregate`](https://reference.langchain.com/javascript/langchain-langgraph/BinaryOperatorAggregate): stores a persistent value, updated by applying a binary operator to the current value and each update sent to the channel, useful for computing aggregates over multiple steps; e.g.,`total = BinaryOperatorAggregate(int, operator.add)`
...
While most users will interact with Pregel through the [StateGraph](https://reference.langchain.com/javascript/langchain-langgraph/StateGraph) API or the [entrypoint](https://reference.langchain.com/javascript/langchain-langgraph/entrypoint) decorator, it is possible to interact with Pregel directly.
...
<Tabs>
    ...
    This example demonstrates how to use the [`BinaryOperatorAggregate`](https://reference.langchain.com/javascript/langchain-langgraph/BinaryOperatorAggregate) channel to implement a reducer.
...
<Tabs>
    ...
    In the [Functional API](/oss/javascript/langgraph/functional-api), you can use an [`entrypoint`](https://reference.langchain.com/javascript/langchain-langgraph/entrypoint) to create a Pregel application. The `entrypoint` decorator allows you to define a function that takes input and returns output.
