...
The most straightforward way to call a model is to use [`invoke()`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel) with a single message or a list of messages.
...
<Accordion title="Advanced streaming topics">
    <Accordion title="Streaming events">
        ...
        See the [`streamEvents()`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel) reference for event types and other details.
...
For more details on batching, see the [reference](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel).
...
To make tools that you have defined available for use by a model, you must bind them using [`bindTools`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel). In subsequent invocations, the model can choose to call any of the bound tools as needed.
...
<AccordionGroup>
    ...
    <Accordion title="Streaming tool calls" icon="rss">
        When streaming responses, tool calls are progressively built through [`ToolCallChunk`](https://reference.langchain.com/javascript/langchain-core/messages/ToolCallChunk). This allows you to see tool calls as they're being generated rather than waiting for the complete response.
...
<Note>
    **Key considerations for structured output:**
    ...
    - **Include raw**: Use [`includeRaw: true`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel) to get both the parsed output and the raw [`AIMessage`](https://reference.langchain.com/javascript/langchain-core/messages/AIMessage)
