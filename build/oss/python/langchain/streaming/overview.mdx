...
To stream agent progress, use the [`stream`](https://reference.langchain.com/python/langgraph/graph/state/CompiledStateGraph) or [`astream`](https://reference.langchain.com/python/langgraph/graph/state/CompiledStateGraph) methods with `stream_mode="updates"`. This emits an event after every agent step.
...
* **LLM node**: [`AIMessage`](https://reference.langchain.com/python/langchain/messages/AIMessage) with tool call requests
* **Tool node**: [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/ToolMessage) with execution result
