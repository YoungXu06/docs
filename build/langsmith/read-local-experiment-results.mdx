...
The [`evaluate()`](https://reference.langchain.com/python/langsmith/client/Client) function returns an [`ExperimentResults`](https://reference.langchain.com/python/langsmith/evaluation/_runner/ExperimentResults) object that you can iterate over. The `blocking` parameter controls when results become available:
